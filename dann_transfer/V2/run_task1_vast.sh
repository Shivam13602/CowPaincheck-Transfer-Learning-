#!/usr/bin/env bash
# V2 protocol: 14 inner folds (2 val cows each), fixed V1 test cows — Task1 DANN + weak-label GCE.
# Defaults tuned for A100-class GPU (batch 16, 12 workers, long epochs, high LR-plateau patience).
# Env overrides: NUM_WORKERS, BATCH_SIZE, DANN_EPOCHS, WEAK_EPOCHS, LR_SCHEDULER_PATIENCE,
# MAX_FRAMES, TRAIN_LR, WEAK_LR, INFER_SLIDING_*, EVAL_MC_SAMPLES, DIAG_BOOTSTRAP_SAMPLES — see MAX_FRAMES_SWEEP.md
# Run from Dann_transfer repo root: bash V2/run_task1_vast.sh
set -euo pipefail
...
V2_TEST_COW_IDS="${V2_TEST_COW_IDS:-363,403,404,408}"
...
echo "== Capacity: NUM_WORKERS=$NUM_WORKERS BATCH_SIZE=$BATCH_SIZE DANN_EPOCHS=$DANN_EPOCHS WEAK_EPOCHS=$WEAK_EPOCHS LR_SCHEDULER_PATIENCE=$LR_SCHEDULER_PATIENCE =="

echo "== V2 Task1-only DANN dry run =="
...
echo "== V2 Task1-only DANN (2 val cows/fold => 14 folds; fixed test cows ${V2_TEST_COW_IDS}) =="
python dann_adapt_v2.9.py \
  --manifest-csv "$TARGET_MANIFEST" \
  --sequence-root "$TARGET_ROOT" \
...
echo "== V2 weak-proxy GCE fine-tune (same CV / test protocol) =="
...
echo "V2 pipeline done. Sync ${OUT_DANN_BASE}*, ${OUT_WEAK_GCE}, and logs."
