#!/usr/bin/env bash
# One-shot: wire /workspace paths to necessary_filesV1 bundle, ensure venv, run Task1 pipeline.
set -eu
BUNDLE="${BUNDLE:-/workspace/necessary_filesV1}"
LOG="${TRAIN_LOG:-/workspace/task1_vast_train.log}"

exec >>"$LOG" 2>&1
echo "=== vast_launch_task1 $(date -u) ==="
