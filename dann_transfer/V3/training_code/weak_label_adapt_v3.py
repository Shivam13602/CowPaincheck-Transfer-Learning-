...
remaining cows are partitioned into inner folds (default was 4 val cows/fold;
use `--val-cows-per-fold 2` for 14 folds per the V2 protocol).
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--dataset-version",
        type=str,
        default="baseline_10s_250",
        help="Human-readable dataset version label written into all V3 reports.",
    )
    p.add_argument(
        "--checkpoint-dir",
        type=Path,
    ...
    p.add_argument(
        ...
        default=True,
        help="Require every validation fold to contain at least one healthy-proxy and one unhealthy-proxy cow.",
    )
    p.add_argument("--fold", type=int, default=None, help="Optional single fold index for smoke runs.")
    ...
    p.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Sample training clips with inverse cow-frequency weights so high-clip cows do not dominate.",
    )
    ...
    p.add_argument(
        "--select-metric",
        choices=("v3_composite", "auc", "cow_auc", "balanced_accuracy_opt", "f1_opt", "f1"),
        default="v3_composite",
        help="Checkpoint selection metric. V3 default = 0.45*seq AUC + 0.35*cow AUC + 0.20*opt balanced accuracy.",
    )
    p.add_argument(
        "--threshold-min-specificity",
        type=float,
        default=0.50,
        help="Minimum validation specificity for the primary pooled threshold; falls back to unconstrained Youden if unmet.",
    )
    p.add_argument("--expected-frames", type=int, default=240, help="Expected frames per 10s sequence for QA rate calculation.")
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional explicit QA audit CSV path. Defaults to out-dir/sequence_qa_audit.csv.",
    )
...
def _apply_cfg_overrides(cfg: Any, args: argparse.Namespace) -> Any:
    ...
    cfg.cow_balanced_sampler = bool(getattr(args, "cow_balanced_sampler", True))
...
def _qa_value(meta: dict[str, Any], names: tuple[str, ...]) -> float | None:
    for name in names:
        if name in meta:
            value = _safe_float(meta.get(name))
...
def _qa_filter_records(
    ...
    expected_frames = max(1, int(getattr(args, "expected_frames", 240) or 240))
    min_detection_rate = float(getattr(args, "qa_min_detection_rate", 0.0) or 0.0)
    max_filled_rate = float(getattr(args, "qa_max_filled_rate", 1.0) or 1.0)
    min_mean_conf = float(getattr(args, "qa_min_mean_confidence", 0.0) or 0.0)
    min_min_conf = float(getattr(args, "qa_min_min_confidence", 0.0) or 0.0)

    kept: list[dict[str, Any]] = []
    audit_rows: list[dict[str, Any]] = []
    for r in records:
        meta = r["meta"]
        detected_frames = _qa_value(meta, ("detected_frames", "n_detected_frames"))
        filled_frames = _qa_value(meta, ("filled_frames", "n_filled_frames"))
        detection_rate = _qa_value(meta, ("detection_rate", "detected_frame_rate"))
        filled_rate = _qa_value(meta, ("filled_rate", "filled_frame_rate"))
        mean_conf = _qa_value(meta, ("mean_detection_confidence", "mean_confidence", "mean_det_conf"))
        min_conf = _qa_value(meta, ("min_detection_confidence", "min_confidence", "min_det_conf"))
        if detection_rate is None and detected_frames is not None:
            detection_rate = float(detected_frames) / float(expected_frames)
        if filled_rate is None and filled_frames is not None:
            filled_rate = float(filled_frames) / float(expected_frames)

        reasons: list[str] = []
        if detection_rate is not None and detection_rate < min_detection_rate:
            reasons.append("low_detection_rate")
        elif detection_rate is None and min_detection_rate > 0.0:
            reasons.append("missing_detection_rate")
        if filled_rate is not None and filled_rate > max_filled_rate:
            reasons.append("high_filled_rate")
        elif filled_rate is None and max_filled_rate < 1.0:
            reasons.append("missing_filled_rate")
        if mean_conf is not None and mean_conf < min_mean_conf:
            reasons.append("low_mean_confidence")
        elif mean_conf is None and min_mean_conf > 0.0:
            reasons.append("missing_mean_confidence")
        if min_conf is not None and min_conf < min_min_conf:
            reasons.append("low_min_confidence")
        elif min_conf is None and min_min_conf > 0.0:
            reasons.append("missing_min_confidence")
        ...
        audit_rows.append(
            {
                ...
                "cow_id": r.get("cow_id"),
                "target": r.get("target"),
                "weak_label_text": meta.get("weak_label_text"),
                "health_condition": meta.get("health_condition"),
    ...
    summary = {
        "dataset_version": str(getattr(args, "dataset_version", "")),
        "expected_frames": int(expected_frames),
        "n_before": int(len(records)),
        "n_after": int(len(kept)),
        "n_excluded": int(len(records) - len(kept)),
        "qa_min_detection_rate": min_detection_rate,
        "qa_max_filled_rate": max_filled_rate,
        "qa_min_mean_confidence": min_mean_conf,
        "qa_min_min_confidence": min_min_conf,
    }
    if not audit_df.empty:
        summary["exclude_reason_counts"] = (
            audit_df.loc[~audit_df["include"], "exclude_reasons"]
            .replace("", "included")
            .value_counts()
            .to_dict()
        )
    if not kept:
        raise RuntimeError("QA filters removed every sequence. Relax QA thresholds or check manifest columns.")
    return kept, audit_df, summary
...
def _build_split_plan(records: list[dict[str, Any]], args: argparse.Namespace) -> dict[str, Any]:
    ...
    if bool(getattr(args, "require_val_both_classes", True)):
        label_by_cow = {str(row.cow_id): int(row.majority_target) for row in cows.itertuples(index=False)}
        bad = [
            (i, sorted(fold), sorted({label_by_cow[c] for c in fold}))
            for i, fold in enumerate(val_folds)
            if len({label_by_cow[c] for c in fold}) < 2
        ]
        if bad:
            details = "; ".join(f"fold {i}: cows={cows_i}, labels={labels_i}" for i, cows_i, labels_i in bad)
            raise ValueError(
                "V3 requires every validation fold to contain both proxy classes. "
                f"Bad folds: {details}. Use --no-require-val-both-classes only for diagnostics."
            )
...
def _print_split_plan(split: dict[str, Any]) -> None:
    ...
    print(f"dataset_version={split.get('dataset_version')} | label_column={split['label_column']} | seed={split['seed']}")
    print(
        "test: "
...
def _threshold_grid(scores: np.ndarray) -> np.ndarray:
    ...
    mids = (vals[:-1] + vals[1:]) / 2.0 if len(vals) > 1 else np.asarray([], dtype=np.float64)
    grid = np.unique(np.concatenate([np.asarray([0.0, 0.5, 1.0]), vals, mids, np.linspace(0.05, 0.95, 19)]))
    return grid.astype(np.float64)


def _threshold_stats(targets: np.ndarray, scores: np.ndarray, threshold: float) -> dict[str, Any]:
    targets = np.asarray(targets, dtype=np.int64)
    scores = np.asarray(scores, dtype=np.float64)
    pred = (scores >= float(threshold)).astype(np.int64)
    tn, fp, fn, tp = [int(x) for x in confusion_matrix(targets, pred, labels=[0, 1]).ravel()]
    ...
    return {
        ...
        "accuracy": float(accuracy_score(targets, pred)) if len(targets) else None,
        "balanced_accuracy": balanced_acc,
        "f1": float(f1_score(targets, pred, zero_division=0)) if len(targets) else None,
        "precision": float(precision_score(targets, pred, zero_division=0)) if len(targets) else None,
        "recall": float(recall_score(targets, pred, zero_division=0)) if len(targets) else None,
        "specificity": specificity,
        "sensitivity": sensitivity,
        "youden_j": (
            float(specificity + sensitivity - 1.0)
            if specificity is not None and sensitivity is not None
            else None
        ),
        "tn": tn,
...
def _threshold_search(targets: np.ndarray, scores: np.ndarray, *, min_specificity: float = 0.50) -> dict[str, Any]:
    targets = np.asarray(targets, dtype=np.int64)
    scores = np.asarray(scores, dtype=np.float64)
    if len(targets) == 0:
        empty = _threshold_stats(targets, scores, 0.5)
        return {
            "fixed_0p5": empty,
            "f1": empty,
            "youden": empty,
            "specificity_constrained": empty,
            "specificity_constraint_met": False,
            "min_specificity": float(min_specificity),
        }
    stats = [_threshold_stats(targets, scores, float(thr)) for thr in _threshold_grid(scores)]

    def finite(value: Any, default: float = float("-inf")) -> float:
        if value is None:
            return default
        value_f = float(value)
        return default if math.isnan(value_f) else value_f

    fixed = _threshold_stats(targets, scores, 0.5)
    best_f1 = max(stats, key=lambda s: (finite(s.get("f1")), finite(s.get("balanced_accuracy")), finite(s.get("specificity")), -s["threshold"]))
    best_youden = max(
        stats,
        key=lambda s: (finite(s.get("youden_j")), finite(s.get("balanced_accuracy")), finite(s.get("specificity")), -s["threshold"]),
    )
    constrained_candidates = [s for s in stats if s.get("specificity") is not None and float(s["specificity"]) >= float(min_specificity)]
    if constrained_candidates:
        constrained = max(
            constrained_candidates,
            key=lambda s: (finite(s.get("youden_j")), finite(s.get("balanced_accuracy")), finite(s.get("sensitivity")), -s["threshold"]),
        )
        constraint_met = True
    else:
        constrained = best_youden
        constraint_met = False
    return {
        "fixed_0p5": fixed,
        "f1": best_f1,
        "youden": best_youden,
        "specificity_constrained": constrained,
        "specificity_constraint_met": bool(constraint_met),
        "min_specificity": float(min_specificity),
    }


def _primary_threshold_from_validation(
    ...
    *,
    ...
    if pred_df.empty or score_col not in pred_df.columns or target_col not in pred_df.columns:
        bundle = _threshold_search(np.array([], dtype=np.int64), np.array([], dtype=np.float64), min_specificity=min_specificity)
    else:
        bundle = _threshold_search(
            pred_df[target_col].values,
            pred_df[score_col].values,
            min_specificity=float(min_specificity),
        )
    primary = dict(bundle.get("specificity_constrained") or bundle.get("youden") or bundle.get("fixed_0p5"))
    bundle["primary_policy"] = "pooled_validation_specificity_constrained_youden"
    bundle["primary_threshold"] = float(primary.get("threshold", 0.5))
    return bundle


def _metrics_at_threshold_bundle(targets: np.ndarray, scores: np.ndarray, bundle: dict[str, Any]) -> dict[str, Any]:
    out: dict[str, Any] = {}
    for name in ("fixed_0p5", "f1", "youden", "specificity_constrained"):
        threshold = float((bundle.get(name) or {}).get("threshold", 0.5))
        out[name] = _binary_metrics(targets, scores, threshold=threshold)
    return out
...
def _binary_metrics(targets: np.ndarray, scores: np.ndarray, *, threshold: float = 0.5) -> dict[str, Any]:
    ...
    if len(targets) == 0:
        return {
            ...
            "youden_balanced_accuracy": None,
            "youden_specificity": None,
            "youden_sensitivity": None,
            "balanced_accuracy_opt": None,
            "specificity_constrained_threshold": None,
            "specificity_constrained_balanced_accuracy": None,
            "specificity_constrained_specificity": None,
            "specificity_constrained_sensitivity": None,
            "specificity_constraint_met": False,
            "tn": 0,
            "fp": 0,
            "fn": 0,
    ...
    threshold_bundle = _threshold_search(targets, scores, min_specificity=0.50)
    youden = threshold_bundle["youden"]
    constrained = threshold_bundle["specificity_constrained"]
    return {
        "n": int(len(targets)),
        ...
        "f1_threshold": float(best_thr),
        "youden_threshold": float(youden["threshold"]),
        "youden_balanced_accuracy": youden.get("balanced_accuracy"),
        "youden_specificity": youden.get("specificity"),
        "youden_sensitivity": youden.get("sensitivity"),
        "balanced_accuracy_opt": youden.get("balanced_accuracy"),
        "specificity_constrained_threshold": float(constrained["threshold"]),
        "specificity_constrained_balanced_accuracy": constrained.get("balanced_accuracy"),
        "specificity_constrained_specificity": constrained.get("specificity"),
        "specificity_constrained_sensitivity": constrained.get("sensitivity"),
        "specificity_constraint_met": bool(threshold_bundle.get("specificity_constraint_met", False)),
        "tn": tn,
        "fp": fp,
        "fn": fn,
        "tp": tp,
...
def _video_key_series(pred_df: pd.DataFrame) -> pd.Series:
    dataset_root = pred_df.get("dataset_root", pd.Series(dtype=str)).astype(str)
    relative_path = pred_df.get("relative_path", pd.Series(dtype=str)).astype(str)
    return dataset_root + "::" + relative_path


def _video_level_metrics(pred_df: pd.DataFrame, *, threshold: float = 0.5) -> tuple[dict[str, Any], pd.DataFrame]:
    if pred_df.empty:
        return _binary_metrics(np.array([], dtype=np.int64), np.array([], dtype=np.float64), threshold=threshold), pd.DataFrame()
    tmp = pred_df.copy()
    tmp["video_key"] = _video_key_series(tmp)
    video_df = (
        tmp.groupby("video_key", as_index=False)
        .agg(
    ...
    video_df["target"] = (video_df["target_mean"] >= 0.5).astype(np.int64)
    metrics = _binary_metrics(video_df["target"].values, video_df["pain_prob"].values, threshold=threshold)
    return metrics, video_df


def _calibrated_video_metrics(pred_df: pd.DataFrame, *, threshold: float = 0.5) -> tuple[dict[str, Any], pd.DataFrame]:
    if pred_df.empty or "pain_prob_calibrated" not in pred_df.columns:
        return _video_level_metrics(pred_df, threshold=threshold)
    tmp = pred_df.copy()
    tmp["pain_prob"] = tmp["pain_prob_calibrated"]
    return _video_level_metrics(tmp, threshold=threshold)
...
def _metric_for_selection(
    ...
    select_metric: str,
    *,
    cow_metrics: dict[str, Any] | None = None,
) -> float:
    cow_metrics = cow_metrics or {}
    if select_metric == "v3_composite":
        seq_auc = _finite_metric(metrics.get("auc"))
        cow_auc = _finite_metric(cow_metrics.get("auc"))
        bacc = None
        for candidate in (
            _finite_metric(metrics.get("specificity_constrained_balanced_accuracy")),
            _finite_metric(metrics.get("balanced_accuracy_opt")),
            _finite_metric(metrics.get("balanced_accuracy")),
        ):
            if candidate is not None:
                bacc = candidate
                break
        if seq_auc is None or cow_auc is None or bacc is None:
            return float("-inf")
        return float(0.45 * seq_auc + 0.35 * cow_auc + 0.20 * bacc)
    if select_metric == "cow_auc":
        value = _finite_metric(cow_metrics.get("auc"))
        return value if value is not None else float("-inf")
    if select_metric == "balanced_accuracy_opt":
        value = None
        for candidate in (
            _finite_metric(metrics.get("specificity_constrained_balanced_accuracy")),
            _finite_metric(metrics.get("balanced_accuracy_opt")),
...
def _make_train_loader(mod: Any, records: list[dict[str, Any]], sequence_root: Path, cfg: Any) -> DataLoader:
    ...
    sampler = None
    if bool(getattr(cfg, "cow_balanced_sampler", True)):
        cow_counts = Counter(str(r["cow_id"]) for r in records)
        class_counts = Counter(int(r["target"]) for r in records)
        weights = [
            1.0 / max(1, cow_counts[str(r["cow_id"])]) / max(1, class_counts[int(r["target"])])
            for r in records
        ]
    ...
    elif bool(getattr(cfg, "use_stratified_sampler", False)) and hasattr(mod, "create_stratified_sampler"):
...
def _train_one_fold(
    *,
    ...
    for epoch in range(int(cfg.num_epochs)):
        ...
        score = _metric_for_selection(val_metrics, str(args.select_metric), cow_metrics=cow_metrics)
        ...
        print(
            ...
            f"cow_auc={cow_metrics.get('auc')} bacc_opt={val_metrics.get('balanced_accuracy_opt')} "
        ...
        if score > best_score:
            ...
            torch.save(
                {
                    "version": "v3_holstein_weak_label_task1",
                    "fold": fold_idx,
                    "epoch": epoch,
...
def _weak_finalize_outputs(
    *,
    ...
    min_specificity = float(getattr(args, "threshold_min_specificity", 0.50))
    raw_threshold_bundle = _primary_threshold_from_validation(
        all_val_pred,
        score_col="pain_prob",
        min_specificity=min_specificity,
    )
    test_threshold = float(raw_threshold_bundle.get("primary_threshold", 0.5))
    ...
    test_video_metrics, test_video_df = _video_level_metrics(test_pred, threshold=test_threshold)
    test_threshold_metrics = _metrics_at_threshold_bundle(
        test_pred["target"].values,
        test_pred["pain_prob"].values,
        raw_threshold_bundle,
    )
    ...
    calibrated_threshold_bundle = _primary_threshold_from_validation(
        all_val_pred,
        score_col="pain_prob_calibrated",
        min_specificity=min_specificity,
    )
    calibrated_test_threshold = float(calibrated_threshold_bundle.get("primary_threshold", 0.5))
    ...
    test_calibrated_video_metrics, test_calibrated_video_df = _calibrated_video_metrics(
        test_pred, threshold=calibrated_test_threshold
    )
    test_calibrated_threshold_metrics = _metrics_at_threshold_bundle(
        test_pred["target"].values,
        test_pred["pain_prob_calibrated"].values,
        calibrated_threshold_bundle,
    )
    ...
    test_video_path = out_dir / "weak_label_cv_test_video_aggregates.csv"
    test_video_df.to_csv(test_video_path, index=False)
    ...
    test_calibrated_video_path = out_dir / "weak_label_cv_test_calibrated_video_aggregates.csv"
    test_calibrated_video_df.to_csv(test_calibrated_video_path, index=False)
    ...
    summary_payload = {
        ...
        "final_test": {
            "dataset_version": str(getattr(args, "dataset_version", "")),
            "threshold_policy": str(raw_threshold_bundle.get("primary_policy")),
            "threshold_min_specificity": min_specificity,
            "threshold_from_pooled_validation_specificity_constrained": test_threshold,
            "raw_validation_thresholds": raw_threshold_bundle,
            "raw_final_test_threshold_metrics": test_threshold_metrics,
            "temperature_from_validation_mean": test_temperature,
            "sequence_metrics": test_metrics,
            "video_metrics": test_video_metrics,
            "cow_metrics": test_cow_metrics,
            "calibrated_threshold_policy": str(calibrated_threshold_bundle.get("primary_policy")),
            "calibrated_threshold_from_pooled_validation_specificity_constrained": calibrated_test_threshold,
            "calibrated_validation_thresholds": calibrated_threshold_bundle,
            "calibrated_final_test_threshold_metrics": test_calibrated_threshold_metrics,
            "calibrated_sequence_metrics": test_calibrated_metrics,
            "calibrated_video_metrics": test_calibrated_video_metrics,
        ...
        "artifacts": {
            ...
            "test_video_aggregates_csv": str(test_video_path),
            "test_cow_aggregates_csv": str(test_cow_path),
            "test_calibrated_video_aggregates_csv": str(test_calibrated_video_path),
    ...
    _write_report(
        ...
        outputs={
            ...
            "calibrated_video_metrics": json.dumps(test_calibrated_video_metrics, default=_json_default),
            "calibrated_cow_metrics": json.dumps(test_calibrated_cow_metrics, default=_json_default),
            "raw_final_test_threshold_metrics": json.dumps(test_threshold_metrics, default=_json_default),
            "calibrated_final_test_threshold_metrics": json.dumps(test_calibrated_threshold_metrics, default=_json_default),
            "raw_validation_thresholds": json.dumps(raw_threshold_bundle, default=_json_default),
            "calibrated_validation_thresholds": json.dumps(calibrated_threshold_bundle, default=_json_default),
...
def _run_weak_eval_only(args: argparse.Namespace) -> int:
    ...
    if args.fold is not None:
        best_paths = [p for p in best_paths if p.parent.name == f"fold_{int(args.fold)}"]
    ...
    if args.fold is not None:
        fold_summaries = [s for s in fold_summaries if int(s.get("fold", -1)) == int(args.fold)]
    ...
    records, _qa_audit_df, qa_summary = _qa_filter_records(records, args)
    run_meta["qa_summary"] = qa_summary
...
def _write_report(
    *,
    ...
    test_video_metrics: dict[str, Any],
    ...
    with report_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Dataset version: `{getattr(args, 'dataset_version', '')}`\n")
        ...
        f.write(
            "- Primary V3 threshold: pooled validation Youden/balanced-accuracy threshold "
            f"with specificity >= `{float(getattr(args, 'threshold_min_specificity', 0.50))}` when feasible.\n"
        )
        ...
        f.write(_markdown_table(pd.DataFrame([test_video_metrics]).round(4)))
        f.write("\n\n## Cow-Level Final Test Metrics (mean prob per animal)\n\n")
        f.write(_markdown_table(pd.DataFrame([test_cow_metrics]).round(4)))
        if "raw_final_test_threshold_metrics" in outputs:
            threshold_metrics = json.loads(outputs["raw_final_test_threshold_metrics"])
            rows = []
            for policy, metrics in threshold_metrics.items():
                row = {"threshold_policy": policy}
                row.update(metrics)
                rows.append(row)
            f.write("\n\n## Raw Final Test Threshold Policies\n\n")
            f.write(_markdown_table(pd.DataFrame(rows).round(4)))
        if "calibrated_sequence_metrics" in outputs:
            ...
            calibrated_video_metrics = json.loads(outputs["calibrated_video_metrics"])
            ...
            f.write(_markdown_table(pd.DataFrame([calibrated_video_metrics]).round(4)))
            f.write("\n\n## Calibrated Cow-Level Final Test Metrics\n\n")
            f.write(_markdown_table(pd.DataFrame([calibrated_cow_metrics]).round(4)))
            if "calibrated_final_test_threshold_metrics" in outputs:
                threshold_metrics = json.loads(outputs["calibrated_final_test_threshold_metrics"])
                rows = []
                for policy, metrics in threshold_metrics.items():
                    row = {"threshold_policy": policy}
                    row.update(metrics)
        ...
        f.write(_markdown_table(test_video_df.round(4)))
        ...
        for key, value in outputs.items():
            if key.startswith("calibrated_") or key in {
                "diagnostics_raw_json",
                "raw_final_test_threshold_metrics",
                "raw_validation_thresholds",
            }:
                continue
            f.write(f"- `{key}`: `{value}`\n")
...
def main() -> int:
    ...
    records = _prepare_records(manifest_csv, sequence_root, str(args.label_column))
    records, qa_audit_df, qa_summary = _qa_filter_records(records, args)
    ...
    best_paths: list[Path] = []
    folds_to_run = list(split["folds"])
    if args.fold is not None:
        folds_to_run = [f for f in folds_to_run if int(f["fold"]) == int(args.fold)]
        if not folds_to_run:
            raise ValueError(f"--fold {args.fold} is not present in split with {len(split['folds'])} folds.")
        print(f"Smoke run: training only fold {args.fold}.")
