#!/usr/bin/env python3
"""
Domain-adversarial UCAPS -> Holstein/Jersey adaptation for v2.9.

This script keeps the Holstein/Jersey cow-held-out protocol from weak_label_adapt_v2.9.py,
but trains each fold with labeled UCAPS source clips plus target-domain Holstein clips.
Target weak labels are optional and low-weight by default; the domain loss can use target
clips without trusting those weak labels as pain ground truth.
"""
...
def _apply_cfg_overrides(cfg: Any, args: argparse.Namespace) -> Any:
    ...
    cfg.cow_balanced_sampler = bool(getattr(args, "target_cow_balanced_sampler", True))
    return cfg
...
def _coral_loss(source_features: torch.Tensor, target_features: torch.Tensor) -> torch.Tensor:
    ...
    sf = source_features.float() - source_features.float().mean(dim=0, keepdim=True)
    tf = target_features.float() - target_features.float().mean(dim=0, keepdim=True)
    source_cov = sf.T.matmul(sf) / float(max(1, sf.size(0) - 1))
    target_cov = tf.T.matmul(tf) / float(max(1, tf.size(0) - 1))
    d = float(max(1, source_features.size(1)))
...
def _train_one_fold(
    *,
    ...
    target_loader = weak._make_train_loader(mod, target_train, target_root, cfg)
    ...
    domain_head = DomainClassifier(_feature_dim(cfg), hidden_dim=int(args.domain_hidden_dim), dropout=float(args.domain_dropout)).to(device)
    ...
    source_task1_auc_init = source_init_metrics.get("source_task1_auc")
    ...
    for epoch in range(int(cfg.num_epochs)):
        ...
        for step in tqdm(range(steps_per_epoch), desc=f"DANN fold {fold_idx}", leave=False, ascii=True):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                ...
                alignment_loss = str(args.alignment_loss)
                ...
                if alignment_loss in {"domain", "domain_coral"}:
                    domain_logits = domain_head(grl(features, lambda_grl))
                    l_domain = F.cross_entropy(domain_logits, domain_targets)
                else:
                    domain_logits = torch.zeros((features.size(0), 2), dtype=features.dtype, device=device)
                    l_domain = features.new_zeros(())
                if alignment_loss in {"coral", "domain_coral"}:
                    l_coral = _coral_loss(sf, tf)
                else:
                    l_coral = features.new_zeros(())
                ...
                loss = (
                    ...
                    + float(args.domain_weight) * l_domain
                    + float(args.coral_weight) * l_coral
            ...
            totals["coral_loss"] += float(l_coral.detach().item())
        ...
        proxy_score = weak._metric_for_selection(val_metrics, str(args.select_metric), cow_metrics=val_cow_metrics)
        source_auc = source_metrics.get("source_task1_auc")
        source_auc_init = source_task1_auc_init
        retention_floor = float(args.source_task1_retention_floor)
        if source_auc_init is not None and not math.isnan(float(source_auc_init)):
            retention_floor = max(retention_floor, float(source_auc_init) - float(args.source_task1_retention_margin))
        source_retention_pass = (
            retention_floor <= 0.0
            or (source_auc is not None and not math.isnan(float(source_auc)) and float(source_auc) >= retention_floor)
        )
        absolute_floor = float(args.source_task1_sanity_floor)
        source_sanity_pass = (
            absolute_floor <= 0.0
            or (source_auc is not None and not math.isnan(float(source_auc)) and float(source_auc) >= absolute_floor)
        )
        score = float(proxy_score) if source_retention_pass else float("-inf")
        ...
        row = {
            ...
            "source_task1_auc_init": source_auc_init,
            "source_task1_retention_floor": float(retention_floor),
            "source_task1_retention_pass": bool(source_retention_pass),
            "source_task1_sanity_floor": float(absolute_floor),
        ...
        print(
            ...
            f"source_auc={source_auc} init={source_auc_init} retention_pass={source_retention_pass} abs70_pass={source_sanity_pass}"
        )
        ckpt_payload = {
            "version": "v3_holstein_dann",
            "fold": fold_idx,
            "epoch": epoch,
            ...
            "source_init_metrics": source_init_metrics,
            "source_task1_auc_init": source_auc_init,
            "source_task1_retention_floor": float(retention_floor),
            "source_task1_retention_pass": bool(source_retention_pass),
            "source_task1_sanity_floor": float(absolute_floor),
        ...
        if source_retention_pass and float(proxy_score) > best_sanity_score:
    ...
    if best_sanity_summary is not None:
        ...
        print(
            f"WARNING fold {fold_idx}: no epoch passed source Task1 retention floor {best_proxy_summary.get('source_task1_retention_floor')}; "
...
def _summary_row(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_task1_auc_init": summary.get("source_task1_auc_init"),
        "source_task1_retention_floor": summary.get("source_task1_retention_floor"),
        "source_task1_retention_pass": summary.get("source_task1_retention_pass"),
...
def _dann_finalize_outputs(
    *,
    ...
    raw_threshold_bundle = weak._primary_threshold_from_validation(
        all_val_pred,
        score_col="pain_prob",
    ...
    test_video_metrics, test_video_df = weak._video_level_metrics(test_pred, threshold=test_threshold)
    test_threshold_metrics = weak._metrics_at_threshold_bundle(
    ...
    calibrated_threshold_bundle = weak._primary_threshold_from_validation(
        all_val_pred,
    ...
    test_calibrated_video_metrics, test_calibrated_video_df = weak._calibrated_video_metrics(
        test_pred, threshold=calibrated_test_threshold
    )
    test_calibrated_threshold_metrics = weak._metrics_at_threshold_bundle(
        test_pred["target"].values,
    ...
    test_video_path = out_dir / "dann_test_video_aggregates.csv"
    ...
    test_calibrated_video_path = out_dir / "dann_test_calibrated_video_aggregates.csv"
...
def _run_dann_eval_only(args: argparse.Namespace) -> int:
    ...
    target_records = weak._prepare_records(manifest_csv, target_root, str(args.label_column))
    target_records, _qa_audit_df, qa_summary = weak._qa_filter_records(target_records, args)
...
def _write_report(
    *,
    ...
    with report_path.open("w", encoding="utf-8") as f:
        ...
        f.write(
            f"- Source Task1 retention gate: AUC >= max(`{args.source_task1_retention_floor}`, "
            f"initial_source_auc - `{args.source_task1_retention_margin}`); absolute `{args.source_task1_sanity_floor}` is diagnostic.\n"
        )
        ...
        f.write(f"- Alignment loss: `{args.alignment_loss}` | domain weight: `{args.domain_weight}` | coral weight: `{args.coral_weight}` | domain lambda max: `{args.domain_lambda_max}`\n")
        f.write(
        ...
        f.write(
            ...
            "`source_task1_retention_pass` = primary source-retention gate; "
            "`checkpoint_selected_from_proxy_fallback` = no epoch passed retention so the best proxy epoch was used._\n\n"
        )
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--target-cow-balanced-sampler", action=argparse.BooleanOptionalAction, default=True)
    ...
    p.add_argument("--source-task1-sanity-floor", type=float, default=0.70, help="Aspirational absolute marker; V3 primary gate is relative retention.")
    p.add_argument("--source-task1-retention-floor", type=float, default=0.55)
    p.add_argument("--source-task1-retention-margin", type=float, default=0.03)
    p.add_argument("--alignment-loss", choices=("domain", "coral", "domain_coral", "none"), default="domain")
    p.add_argument("--domain-weight", type=float, default=0.10)
    p.add_argument("--coral-weight", type=float, default=0.10)
    ...
    p.add_argument("--threshold-min-specificity", type=float, default=0.50)
...
def main() -> int:
    ...
    weak = _load_weak_module()
    target_records = weak._prepare_records(manifest_csv, target_root, str(args.label_column))
    target_records, qa_audit_df, qa_summary = weak._qa_filter_records(target_records, args)
    ...
    best_paths: list[Path] = []
    folds_to_run = list(target_split["folds"])
    if args.fold is not None:
        folds_to_run = [f for f in folds_to_run if int(f["fold"]) == int(args.fold)]
        if not folds_to_run:
            raise ValueError(f"--fold {args.fold} is not present in split with {len(target_split['folds'])} folds.")
