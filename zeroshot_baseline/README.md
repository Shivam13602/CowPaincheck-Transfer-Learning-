# Phase 0 — Zero-Shot and Weak-Label Baselines

**Status:** Completed pre-DANN archive · **Dataset:** [`baseline_10s_250`](../datasets/baseline_10s_250/)

This folder archives UCAPS v2.9 → Holstein/Jersey transfer **before** DANN/SSL domain adaptation. Direct transfer and ordinary fine-tuning showed weak proxy separation (seq AUC ~0.45–0.55; cow AUC ~0.50).

## Results

![Phase 0 baselines](../docs/figures/phase0_baselines.png)

| Stage | Final seq AUC | Cow AUC |
|-------|-------------:|--------:|
| Zero-shot | 0.529 | ~0.50 |
| Freeze-CNN weak FT | 0.548 | 0.500 |
| Full-backbone weak FT | 0.457 | 0.500 |
| SSL → weak FT | 0.447 | 0.500 |

## Pipeline

```mermaid
flowchart LR
  ucaps[UCAPS v2.9 pretrained] --> zs[Zero-shot eval on 250 seq]
  ucaps --> freeze[Freeze-CNN weak FT]
  ucaps --> full[Full-backbone weak FT]
  ssl[SimSiam SSL] --> sslw[SSL then weak FT]
  freeze --> test[4-cow held-out test]
  full --> test
  sslw --> test
```

Next → [**V1**](../dann_transfer/V1/README.md): Task1 DANN + domain adaptation.
